...
;(function($){
	...
	var TreeGrid = function(_target, options){
		...
		var show_child_count = settings.show_child_count || false;
		var show_row_index = settings.show_row_index || false;
		...
		drowRowData = function(_rows, _level, _pid){
			...
			for(var i=0; i<_rows.length; i++){
				...
				for(var j=0; j<_cols.length; j++){
					...
					if(j==0){
						if(chk_show&&chk_node_next){
							str += "<input type='checkbox' trid='r" + trid + "' value='" + idValue + "'>";
						}
						if(dynamic_load_data){
					...
					if(col.handler){
						if(typeof col.handler=='string'){
							dealData =(eval(col.handler + ".call(new Object(), 'r'+trid, cellValue, row, col)") || "");
						}else{
							dealData = col.handler(row, col) ;
						}
						str += dealData + "</td>";
					}else{
						if(j==0){
